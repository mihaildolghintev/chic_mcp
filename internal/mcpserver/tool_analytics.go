...
func registerComparePeriods(s *server.MCPServer, api MoyskladAPI) {
	tool := mcp.NewTool("compare_periods",
		mcp.WithDescription(
	...
	s.AddTool(tool, func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		optsA := moysklad.ProfitOptions{From: req.GetString("period_a_from", ""), To: req.GetString("period_a_to", ""), Limit: 1000}
		optsB := moysklad.ProfitOptions{From: req.GetString("period_b_from", ""), To: req.GetString("period_b_to", ""), Limit: 1000}
...
func registerABCAnalysis(s *server.MCPServer, api MoyskladAPI) {
	...
	s.AddTool(tool, func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		opts := moysklad.ProfitOptions{From: from, To: to, Limit: 1000}
		metric := req.GetString("metric", "revenue")
		...
		case "product", "":
			...
			return resultOrError(aggregate.ABC(lines, productMetric(metric), 0.8, 0.95), nil)
		case "counterparty":
			...
			return resultOrError(aggregate.ABC(lines, entityMetric(metric), 0.8, 0.95), nil)
		default:
			return mcp.NewToolResultError("dimension must be product or counterparty"), nil
		}
	})
...
func registerSegmentCounterparties(s *server.MCPServer, api MoyskladAPI) {
	tool := mcp.NewTool("segment_counterparties",
		mcp.WithDescription(
	...
	s.AddTool(tool, func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		rows, err := api.GetCounterpartyReport(ctx, nil, clampLimit(req.GetInt("limit", 1000)))
		if err != nil {
			return resultOrError[any](nil, err)
		}
		...
		return resultOrError(aggregate.SegmentCounterparties(rows, params), nil)
	})
}
...
func registerDeadStock(s *server.MCPServer, api MoyskladAPI) {
	...
	s.AddTool(tool, func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		stock, err := api.GetStock(ctx, moysklad.StockOptions{StockMode: "positiveOnly", GroupBy: "product", Limit: 1000})
		if err != nil {
			return resultOrError[any](nil, err)
		}
		...
		from, to := dateArgs(req)
		if from != "" || to != "" {
			turns, err := api.GetTurnover(ctx, moysklad.ProfitOptions{From: from, To: to, Limit: 1000})
			if err != nil {
				return resultOrError[any](nil, err)
			}
		...
		return resultOrError(aggregate.DeadStock(stock, outcomeByRef, threshold), nil)
	})
}
...
func registerReceivablesAging(s *server.MCPServer, api MoyskladAPI) {
	tool := mcp.NewTool("receivables_aging",
		mcp.WithDescription(
	...
	s.AddTool(tool, func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		docs, err := api.SearchDocuments(ctx, moysklad.DocInvoiceOut, moysklad.DocumentQuery{
			Expand: []string{"agent"},
			Limit:  clampLimit(req.GetInt("limit", 1000)),
			Order:  "moment,desc",
		})
		if err != nil {
			return resultOrError[any](nil, err)
		}
		return resultOrError(aggregate.ReceivablesAging(docs, time.Now()), nil)
	})
}
